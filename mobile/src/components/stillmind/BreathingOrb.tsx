...
export function BreathingOrb({ phase, seconds, compact = false, paused = false, elapsedSeconds }: { paused?: boolean; elapsedSeconds?: number; phase?: "吸气" | "呼气"; seconds?: number; compact?: boolean }) {
  ...
  const renderedScale = reduceMotion ? 1 : elapsedSeconds === undefined ? scale : 1.01 - 0.07 * Math.cos((elapsedSeconds % 6) / 6 * Math.PI * 2);
  ...
  return (
    <View style={[styles.stage, { height: size + 64 }]} accessible accessibilityLabel={phase ? `${phase}，剩余 ${seconds ?? 0} 秒` : "观察者光圈"}>
      ...
      <Animated.View style={{ transform: [{ scale: renderedScale }] }}>
