...
- Added a GTM readiness guard that verifies the 15-user seed protocol, CSV schema, analyzer command, analyzer blank-template behavior, six-week validation loop, KPI targets, privacy-safe analytics plan, and human go/no-go gates stay aligned before a broader TestFlight or App Store launch.
- Added a source-material boundary document and guard for 古零/《意识强度》 inputs: the 906-page local corpus is recorded as internal source material, allowed product translations are explicit, and release checks now block drift toward consciousness scores, fixed ego labels, diagnosis/treatment claims, awakening guarantees, or public source-derived authority.
...
- Added `npm run status:launch`, a repository-grounded status reporter that separates completed engineering gates from EAS, Apple, legal, real-device, and seed-user external gates. It now also supports `--live` probes for local git cleanliness, Expo/EAS login, EAS project linkage, and the production seed-test URL.
