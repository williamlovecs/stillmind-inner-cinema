...
function MethodExperience({ methodId, instruction, secondsLeft, stepIndex }: { methodId: MethodId; instruction: string; secondsLeft: number; stepIndex: number }) {
  ...
  if (methodId === "anchors") {
    return <PerspectiveZoomEngine instruction={instruction} stepIndex={stepIndex} />;
  }
  ...
  if (methodId === "wide-gaze") {
    ...
    return (
      <div className="relative grid flex-1 place-items-center overflow-hidden rounded-[2rem] border border-sky-200/15 bg-[radial-gradient(circle_at_50%_24%,rgba(125,211,252,0.12),transparent_44%),rgba(2,6,23,0.76)] p-6 text-center">
        ...
        <div className="mt-3 grid w-full max-w-md grid-cols-2 gap-2"><div className="rounded-2xl border border-sky-100/15 bg-sky-100/[0.07] p-3"><p className="text-xs text-sky-100/55">连续凝视</p><p className="mt-1 text-2xl font-semibold text-sky-50">{steadySeconds}s</p></div><div className="rounded-2xl border border-white/10 bg-white/[0.04] p-3"><p className="text-xs text-stone-500">回归次数</p><p className="mt-1 text-2xl font-semibold text-white">{returns}</p></div></div>
      </div>
    );
  }
